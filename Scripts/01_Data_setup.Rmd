...
growth91_data <- left_join(growth91_data_nobday, birthdates_data, by = c("basebrgy_basewman")) %>% 
  ...
  mutate( fatfree_mass = weghtndx-fatmass_91)
...
growth02_data <- left_join(screen_data, anthdiet, by = c("basebrgy_basewman")) %>% 
  ...
  mutate(fatmass = if_else(sexchild == "1", fatmass_m_02, fatmass_f_02)) %>%
  select(-c(fatmass_f_02, fatmass_m_02)) %>% 
  mutate (fatfree_mass_02 = weight_02-fatmass)
...
anthdiet05 <-anthdiet05 %>% 
  ...
  mutate(fatmass = if_else(sex == "1", fatmass_m_05, fatmass_f_05)) %>%
  select(-c(fatmass_f_05, fatmass_m_05)) %>% 
  mutate (fatfree_mass_05 = weight_05-fatmass)
...
growth_data<- growth_data_unfiltered %>% 
   unite(basebrgy, basewman, sep = "_", col = "basebrgy_basewman") %>% 
  select(uncchdid, basebrgy_basewman, sexchild, sex, outcome, birthdate, intwdate, age_days_birthweigh, weightak_kg, heightcm, date_inf_meas, age_days_infweigh, hght_12, wght_12, intwdate91, age91_days, age_mo_91, age91_years, hightndx, weghtndx, intw_date02, age_days_02, age_years_02, age_cutoff, age_cutoff_days, height_02, weight_02, age, height_05, height_m_05, weight_05, bmi_05, waist_05, tricep1_05, tricep2_05, tricep3_05, subscap1_05, subscap2_05, subscap3_05,hfaz_birth, hfaz_inf_12, hfaz_91, hfaz_02, hfa_diff_birth_inf12, hfa_diff_inf12_91, hfa_diff_91_02, wfaz_birth, wfaz_inf_12, wfaz_91, wfa_diff_birth_inf12, wfa_diff_inf12_91, reprostat, was_preg_no_na, trimester) %>% 
