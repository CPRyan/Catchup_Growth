...
weight_mlong_bday <-mlong_anthro_bday %>%
  select(basebrgy_basewman, sexchild, weight_kg, date_inf_meas, age_days_infweigh, birthdate) %>%
...
weight_mlong_bday_1 <- weight_mlong_bday %>% 
...
weight_mlong_bday_1 <- addWGSR(data = weight_mlong_bday_1, sex = "sexchild", firstPart= "1", secondPart = "age_days_infweigh", index = "wfa") %>% 
  rename( wfaz_inf_1 = wfaz)
  
weight_mlong_bday_12 <- addWGSR(data = weight_mlong_bday_12, sex = "sexchild", firstPart= "12", secondPart = "age_days_infweigh", index = "wfa") %>% 
  rename( wfaz_inf_12 = wfaz)

weight_mlong_bday_z <-left_join(weight_mlong_bday_1, weight_mlong_bday_12, by = c("basebrgy_basewman","sexchild","age_days_infweigh", "date_inf_meas", "birthdate"))

weight_mlong_bday_z

#height z scores infancy

height_mlong_bday_1 <- height_mlong_bday %>% 
  select(basebrgy_basewman, sexchild, date_inf_meas, age_days_infweigh, birthdate, "1")
...
height_mlong_bday_1 <- addWGSR(data = height_mlong_bday_1, sex = "sexchild", firstPart= "1", secondPart = "age_days_infweigh", index = "hfa") %>% 
  rename( hfaz_inf_1 = hfaz)
  
height_mlong_bday_12 <- addWGSR(data = height_mlong_bday_12, sex = "sexchild", firstPart= "12", secondPart = "age_days_infweigh", index = "hfa") %>% 
  rename( hfaz_inf_12 = hfaz)

height_mlong_bday_z <-left_join(height_mlong_bday_1, height_mlong_bday_12, by = c("basebrgy_basewman","sexchild","age_days_infweigh", "date_inf_meas", "birthdate"))
...
growth83_86_infdata <-left_join(weight_mlong_bday_z, height_mlong_bday_z, by = c("basebrgy_basewman","sexchild","age_days_infweigh", "date_inf_meas", "birthdate"))
...
ggplot(growth02_data, aes(x=height))+
  geom_histogram(color = "white")+
  facet_wrap(~sexchild)

#visualize weight distribution 
ggplot(growth02_data, aes(x=weight))+
  geom_histogram(color = "white")+
  facet_wrap(~sexchild)
...
summary_height_02 <- growth02_data %>%
  group_by(sexchild) %>% 
  drop_na() %>%
  summarize(mean_height02 = mean(height),
            std_dev_height02 = sd(height),
            med_height02 = median(height),
            max_height02 = max(height),
            min_height02 = min(height),
            count = n(), .groups = "keep")
summary_height_02
...
summary_weight_02 <- growth02_data %>%
  group_by(sexchild) %>% 
  drop_na() %>%
  summarize(mean_weight02 = mean(weight),
            std_dev_height02 = sd(height),
            med_weight02 = median(weight),
            max_weight02 = max(weight),
            min_weight02 = min(weight),
            count = n(), .groups = "keep")
...
```{r, chache=TRUE}
#load anthdiet 05
anthdiet05 <-read_dta(here::here("Projects/Catchup_Growth/Data/zip_child_2005/anthdiet.dta")) %>% # note: here::here calls the here function
  rename_all(tolower)
...
growth05_data <- left_join(birthdates_data, anthdiet05, by = c("basebrgy_basewman")) %>% 
  ...
  mutate( bmi_05 = (weight/(height_m^2))) %>% 
  filter(weight > 0) %>% 
  filter(height > 0)
...
summary_height_05 <- growth05_data %>%
  group_by(sexchild) %>% 
  drop_na() %>%
  summarize(mean_height05 = mean(height),
            std_dev_height05 = sd(height),
            med_height05 = median(height),
            max_height05 = max(height),
            min_height05 = min(height),
            count = n(), .groups = "keep")
summary_height_05
...
summary_weight_05 <- growth05_data %>%
  group_by(sexchild) %>% 
  drop_na() %>%
  summarize(mean_weight05 = mean(weight),
            std_dev_height05 = sd(height),
            med_weight05 = median(weight),
            max_weight05 = max(weight),
            min_weight05 = min(weight),
            count = n(), .groups = "keep")
...
zscore_wfa<- zscore_data %>% 
  select(basebrgy_basewman, sexchild, age_days_birthweigh, age_days_infweigh, age91_days, wfaz_birth, wfaz_inf_1, wfaz_inf_12, wfaz_91)

zscore_hfa <-zscore_data %>% 
  select(basebrgy_basewman, sexchild, age_days_birthweigh, age_days_infweigh, age91_days, hfaz_birth, hfaz_inf_1, hfaz_inf_12, hfaz_91, hfaz_02) 
...
hfa_diff_birth_inf12 <- zscore_hfa %>% 
  select(basebrgy_basewman, sexchild, hfaz_birth, hfaz_inf_12) %>%
  mutate(hfa_diff_inf12_91 = (hfaz_inf_12 - hfaz_birth))

#make a table for 2 yrs old - 91 data
#find the difference between these

hfa_diff_inf12_91 <- zscore_hfa %>% 
...
wfa_diff_birth_inf12 <- zscore_wfa %>% 
...
zscore_hfa_diffs <- left_join(hfa_diff_birth_inf12, hfa_diff_inf12_91, by = c("basebrgy_basewman", "sexchild")) 
zscore_hfa_diffs <- left_join(hfa_diff_inf12_91, hfa_diff_91_02, by = c("basebrgy_basewman", "sexchild", "hfaz_91"))

zscore_hfa_diffs

zscore_wfa_diffs <- left_join(wfa_diff_birth_inf12, wfa_diff_inf12_91, by = c("basebrgy_basewman", "sexchild", "wfaz_inf_12")) 
