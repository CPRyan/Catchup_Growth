...
dim(hfa_diff1)
dim(hfa_diff2)
dim(zscore_hfa_diffs)
# OK, we're introducing extra rows here somehow. hfa_diffs1 and hfa_diffs2 both have 3069, but zscore_hfa_diffs has 3161
# 
intersect(names(hfa_diff1), names(hfa_diff2))
# Ok, at least this one place we are including 2 variables for hfaz_inf_12

zscore_hfa_diffs <- left_join(hfa_diff1, hfa_diff2 %>% 
                                select(-hfaz_inf_12), by = c("basebrgy_basewman", "sexchild")) 
dim(zscore_hfa_diffs)
# Still 3161

intersect(names(zscore_hfa_diffs), names(hfa_diff3))

zscore_hfa_diffs <- left_join(zscore_hfa_diffs, hfa_diff3 %>% 
                                select(-hfaz_91), 
                              by = c("basebrgy_basewman", "sexchild")) %>% 
  select(basebrgy_basewman, sexchild, hfaz_birth, hfaz_inf_12, hfaz_91, hfaz_02, hfa_diff_birth_inf12, hfa_diff_inf12_91, hfa_diff_91_02)

zscore_hfa_diffs
# Dang, we have yet more rows. We're definitely adding artifacts.
...
intersect(names(zscore_hfa_diffs), names(zscore_wfa_diffs))


zscore_diffs <- left_join(zscore_hfa_diffs, zscore_wfa_diffs %>% select(-sexchild), by = c("basebrgy_basewman"))
# Ok, we're introducing another 1000 almost here. This is a problem...

zscore_diffs %>% filter(basebrgy_basewman == "1_178")
zscore_hfa_diffs %>% filter(basebrgy_basewman == "1_178")
zscore_wfa_diffs %>% filter(basebrgy_basewman == "1_178")
...
# minimal just clock data and z scores? need to filter sex later
