...
growth83_86_birthdata<-mbirth2 %>% 
 ...
 growth83_86_birthdata <- growth83_86_birthdata %>%  
  ...
  mutate(birthdate = make_date(yearbirt + 1000, mnthbrth, daybirth)) %>% 
  mutate(intwdate = make_date(yearintw +1000, mothintw, dayintw)) %>% 
...
```{r, cache=TRUE}
 mlong_anthro <-mlong %>% 
  ...
  mutate(date_inf_meas = lubridate::make_date(year = yearms+1000,
                                              month = mntham,
                                              day = daymes)) %>% 
...
growth91_data<- growth91_data %>% 
  mutate(intwdate91 = make_date( yrintr91+1000, mointr91, dyintrvw)) %>% 
...
anthdiet <- left_join(birthdates_data, anthdiet, by = c("basebrgy", "basewman"))
